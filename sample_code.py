...
def validate_phone_numbers(phone_numbers, names):
    ...
    for phone_number in phone_numbers:
        ...
        if len(digits_only_phone_number) < 10:
            raise IncorrectPhoneNumberLength(f'phone number \'{phone_number}\' for {name} is too short and unable to be resolved')
        
        if len(digits_only_phone_number) > 13:
            raise IncorrectPhoneNumberLength(f'phone number \'{phone_number}\' for {name} is too long and unable to be resolved')
...
def validate_emails(emails, names):
    ...
    for email in emails:
        ...
        if not pattern.match(email):
            ...
            raise InvalidEmailAddress(f'email \'{email}\' for {name} is not a valid email, ensure it follows Username@EmailServer.TLD like example@gmail.com. If this email is valid, please change the regular expression')
...
def validate_linkedins(linkedins, names):
    ...
    for linkedin in linkedins:
        ...
        if not any(substring in linkedin_lower for substring in link_characters):
            ...
            raise InvalidLinkedIn(f'LinkedIn account \"{linkedin_lower}\" for {name} is invalid, please ensure it is inputted correctly')
            ...
            try:
                ...
                raise InvalidLinkedIn(f'LinkedIn account \"{linkedin_lower}\" for {name} is missing a handle, please ensure it is inputted correctly')
